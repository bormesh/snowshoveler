...
from snowshoveler import configwrapper

log = logging.getLogger('snowshoveler.upgrade_database')
...
def get_scripts_already_ran(pgconn):
    ...
    cursor.execute(textwrap.dedent("""
        select script_path
        from snowshoveler_schema_version
        """))
...
def run_script(pgconn, path_to_script):
    ...
    try:
        ...
        cursor.execute(textwrap.dedent("""
            insert into snowshoveler_schema_version
...
def maybe_run_setup_script(pgconn, setup_script):
    ...
    """
    Check if the snowshoveler_schema_version table exists.
    ...
    cursor.execute(textwrap.dedent("""
        select exists(
            ...
            where relname = 'snowshoveler_schema_version'
            and relkind = 'r'
        )
        """))
    ...
    if cursor.fetchone().exists:
        ...
        log.info("About to create snowshoveler_schema_version table.")
